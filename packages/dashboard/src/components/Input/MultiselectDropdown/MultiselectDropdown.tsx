  ...
  const Row = ({
    ...
    return (
      <li
        ...
        onClick={() => {
          const isAlreadySelected = selectedValues.includes(value.value);
      ...
      >
        {value.tag && (
          <span
            className={
              "mr-3 whitespace-nowrap rounded bg-blue-100 px-3 py-0.5 text-xs text-blue-900"
            }
          >
            {value.tag}
          </span>
        ...
        {value.value ===
        selectedValues.find((selection) => value.value === selection) ? (
          ...
          <AnimatePresence>
            {open && (
              <motion.div
                ...
                className="absolute z-40 w-full p-1 pr-1 mt-1 space-y-3 overflow-y-scroll text-base list-none bg-white border border-black rounded-md shadow scrollbar-w-2 scrollbar scrollbar-thumb-rounded-full scrollbar-thumb-neutral-400 scrollbar-track-neutral-100 max-h-72 border-opacity-5 focus:outline-none sm:text-sm"
                tabIndex={-1}
                role="listbox"
              >
                <li className="relative px-3 py-2 cursor-default select-none text-neutral-800">
                  <input
                    type="search"
                    name="search"
                    autoComplete={"off"}
                ...
                {filteredValues.length === 0 ? (
                  <li className="relative py-2 pl-3 cursor-default select-none pr-9 text-neutral-800">
                    No results found
                  </li>
                ) : (
                  <List
