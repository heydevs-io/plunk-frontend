...
export default function Index() {
  ...
  const methods = useZodForm(CampaignSchemas.update, {
    ...
    watch((value, { name, type }) => {
      if (name === "email") {
        if (
          ...
          setError("email", {
            type: "manual",
            message: `The sender address must end with @${
              project.email?.split("@")[1]
            }`,
          });
        } else {
          clearErrors("email");
  ...
  const send: FirstParams<typeof handleSubmit> = async (data) => {
    ...
    await network.mock<Campaign, typeof CampaignSchemas.update>(
      ...
      "/v1/campaigns",
      {
        // @ts-expect-error: `id` is not explicitly defined in `campaign` with any type
        id: campaign.id,
        ...data,
        recipients:
  ...
  const sendTest: typeof send = async (data) => {
    await network.mock<Campaign, typeof CampaignSchemas.update>(
      project.secret,
      "PUT",
      "/v1/campaigns",
        ...
        <Card
          ...
          options={
            <>
              <button
                onClick={duplicate}
                className="flex items-center w-full gap-2 px-4 py-2 text-sm transition text-neutral-700 hover:bg-neutral-100"
                role="menuitem"
                tabIndex={-1}
              >
        ...
        >
          <FormProvider {...methods}>
            <form
              ...
              className="grid gap-6 space-6 sm:grid-cols-6"
            >
              ...
              {contacts ? (
                <>
                  <div className={"sm:col-span-3"}>
                    ...
                    />
                    <AnimatePresence>
                      {(errors.recipients as FieldError | undefined)
                        ?.message && (
                        <motion.p
                          initial={{ height: 0 }}
                          animate={{ height: "auto" }}
                  ...
                  campaign.status !== "DELIVERED" && (
                    ...
                    >
                      <Alert type={"info"} title={"Automatic batching"}>
                        Your campaign will be sent out in batches of 80
                        recipients each. It will be delivered to all contacts{" "}
                        {dayjs().to(
                          dayjs().add(
              ...
              <EditorSwitcher modeSwitcher={campaign} />

              <div
                className={
                  "ml-auto mt-6 flex justify-end gap-x-5 sm:col-span-6"
                }
              >
                {campaign.status === "DRAFT" ? (
                  <>
